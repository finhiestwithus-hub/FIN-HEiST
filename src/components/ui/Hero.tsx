...
export default function Hero({ onOpenModal }: HeroProps) {
  const [activeTab, setActiveTab] = useState<'itr' | 'gst' | 'project' | 'compliance'>('itr');
  ...
  const trustBadges = [
    { icon: '✔', label: 'GST Experts' },
    { icon: '✔', label: 'ITR Specialists' },
  ...
  const dashboardTabs = [
    ...
    { id: 'project', label: 'Loan Reports', icon: BarChart3 },
  ...
  return (
    <section
      id="home"
      className="relative overflow-hidden min-h-[94vh] lg:min-h-screen w-full flex items-center justify-between pt-28 pb-16 lg:pt-32 lg:pb-20 bg-mesh-hero"
    >
      ...
      <div className="absolute bottom-10 right-1/4 w-[550px] h-[550px] rounded-full bg-yellow-400/25 blur-[150px] pointer-events-none z-0 animate-blob float-delay-2" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full bg-orange-300/20 blur-[170px] pointer-events-none z-0 animate-blob float-delay-4" />
      ...
      <div className="max-w-[1760px] mx-auto px-4 sm:px-8 lg:px-12 xl:px-16 relative z-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 xl:gap-16 items-center w-full">
          ...
          <div className={`lg:col-span-7 xl:col-span-6 space-y-7 transition-all duration-1000 ease-out transform ${
            isLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-16'
          }`}>

            {/* Top Badge */}
            <div className={`inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-gradient-to-r from-slate-900 to-slate-800 text-amber-300 text-xs font-bold shadow-md shadow-amber-500/20 border border-amber-500/40 animate-card-tilt transition-all duration-700 delay-100 transform ${
              isLoaded ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 -translate-y-6 scale-90'
            }`}>
              <Sparkles className="w-3.5 h-3.5 text-amber-400 animate-spin-slow" />
              <span>Chartered Accountants & Financial Services — Dehradun & Pan-India</span>
            </div>

            {/* Main Headline */}
            <h1 className={`text-4xl sm:text-5xl lg:text-6xl font-extrabold font-poppins tracking-tight leading-[1.12] text-slate-900 transition-all duration-700 delay-200 transform ${
            ...
            {/* Subheadline */}
            <p className={`text-lg text-slate-600 font-inter leading-relaxed max-w-2xl transition-all duration-700 delay-300 transform ${
              isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
            }`}>
              <strong className="text-slate-900 font-semibold">FIN-HEIST</strong> is a CA practice led by Dual CA Finalists, delivering Income Tax returns, GST compliance, Company registration, and bank-accepted CMA & DSCR project reports across India.
            </p>

            {/* CTA Buttons */}
            <div className={`flex flex-wrap items-center gap-4 pt-2 transition-all duration-700 delay-500 transform ${
              isLoaded ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-10 scale-95'
            }`}>
              <button
                onClick={() => onOpenModal('Book Consultation')}
                className="px-8 py-4 rounded-xl bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-600 hover:from-amber-600 hover:to-yellow-600 text-slate-950 font-poppins font-extrabold text-base shadow-xl shadow-amber-500/35 hover:shadow-amber-500/50 transition-all duration-300 transform hover:-translate-y-1 flex items-center gap-2.5 group border border-amber-300/40 animate-card-pulse"
              >
              ...
              <a
                href="#services"
                className="px-7 py-4 rounded-xl glass-card hover:bg-white text-slate-800 hover:text-amber-600 font-semibold transition-all duration-300 transform hover:-translate-y-0.5 flex items-center gap-2 text-base shadow-md hover:shadow-lg border border-slate-300/80 animate-card-diagonal float-delay-1"
              >
              ...
              <a
                ...
                className="px-6 py-4 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-semibold transition-all duration-300 transform hover:-translate-y-0.5 flex items-center gap-2 text-base shadow-lg shadow-emerald-500/25 animate-card-bob float-delay-2"
              >
            ...
            {/* Trust Badges */}
            <div className={`pt-4 border-t border-slate-200 transition-all duration-700 delay-700 transform ${
              isLoaded ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-6 scale-90'
            }`}>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                {trustBadges.map((badge, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 px-3.5 py-2.5 rounded-xl shadow-sm hover:border-amber-400 hover:shadow-md transition-all duration-300"
                  >
            ...
            {/* Live Stats */}
            <div className={`flex items-center gap-6 text-xs text-slate-500 font-inter transition-all duration-700 delay-900 transform ${
          ...
          <div className={`lg:col-span-5 xl:col-span-6 relative transition-all duration-1000 ease-out delay-300 transform ${
            isLoaded ? 'opacity-100 translate-y-0 translate-x-0 scale-100 rotate-0' : 'opacity-0 translate-y-20 translate-x-12 scale-90 rotate-2'
          }`}>

            {/* Floating accent badge */}
            <div className="hidden sm:flex absolute -top-5 -right-4 px-4 py-2.5 rounded-2xl bg-white border border-amber-200 text-xs font-bold text-amber-700 shadow-lg items-center gap-2 z-20 animate-card-bob float-delay-0">
              <ShieldCheck className="w-4 h-4 text-amber-600" />
              <span>CA Certified Workflow</span>
            </div>

            <div className="hidden sm:flex absolute -bottom-4 -left-4 px-4 py-2.5 rounded-2xl bg-white border border-emerald-200 text-xs font-bold text-emerald-700 shadow-lg items-center gap-2 z-20 animate-card-diagonal float-delay-2">
              <Sparkles className="w-4 h-4 text-emerald-500" />
              <span>Direct CA Finalist Access</span>
            </div>

            {/* Dashboard Frame */}
            <div className="group relative rounded-3xl bg-white border-2 border-slate-200 shadow-2xl shadow-slate-300/80 overflow-hidden animate-float-slow float-delay-1 transition-all duration-500 hover:border-amber-400 hover:shadow-[0_30px_80px_-15px_rgba(245,158,11,0.3)]">
              ...
              {/* Tab Navigation */}
              <div className="grid grid-cols-4 gap-0 border-b border-slate-200 bg-slate-50 relative z-10">
                {dashboardTabs.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center py-3 px-1 text-[11px] font-bold transition-all border-b-2 ${
                        isActive
                ...
                {activeTab === 'project' && (
                  <div className="space-y-4 animate-fadeIn">
                    <div className="flex items-center justify-between bg-amber-50 border border-amber-200 p-3.5 rounded-xl">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-amber-600 text-white flex items-center justify-center font-bold font-mono text-xs">
                          DSCR
                        </div>
                        <div>
                          <h4 className="text-sm font-bold text-slate-900">Bank Loan Project Report</h4>
                          <p className="text-xs text-slate-500">MSME & Term Loan Dossier</p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { label: 'Calculated DSCR', val: '2.14x', color: 'emerald', note: 'Optimal for ₹3.5 Cr loan' },
                        { label: 'CMA Sheets', val: '7-Yr', color: 'amber', note: 'RBI format verified' }
                      ].map(item => (
                        <div key={item.label} className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
                          ...
                          <div className={`text-lg font-bold ${item.color === 'emerald' ? 'text-emerald-600' : 'text-amber-600'} font-mono`}>{item.val}</div>
                          <span className="text-[10px] text-slate-400">{item.note}</span>
                        </div>
                      ))}
                    </div>
                    <div className="p-3 rounded-xl bg-amber-50 border border-amber-100 text-xs text-slate-700 flex items-center justify-between">
                      <span>Projections, MPBF & Sensitivity Graph</span>
                      <span className="text-amber-600 font-bold flex items-center gap-1">Exported <ArrowUpRight className="w-3 h-3" /></span>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-3.5 animate-fadeIn">
                    ...
                    <div className="space-y-2 text-xs">
                      {[
                        ...
                        { text: 'Advance Tax Q2 Calculation', status: 'Assisted', color: 'amber' },
                      ].map(item => (
