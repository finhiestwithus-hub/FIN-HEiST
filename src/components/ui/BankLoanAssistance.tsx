...
export default function BankLoanAssistance({ onOpenModal }: BankLoanAssistanceProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (tabParam && ['itr', 'gst', 'finance', 'tds-tcs'].includes(tabParam)) {
...
`}} />
      ...
      <div className="max-w-[1760px] w-full mx-auto px-4 sm:px-8 lg:px-12 xl:px-16 relative z-10">
        ...
        <div className="text-center mb-10">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold font-poppins tracking-tight mb-4 text-transparent bg-clip-text bg-gradient-to-r from-amber-600 via-yellow-500 to-amber-600 drop-shadow-sm pb-1">
            Calculator
          </h2>
            ...
            {UTILITY_CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`relative z-10 flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-full text-xs sm:text-sm font-bold transition-colors w-auto sm:w-40 shrink-0 ${activeTab === cat.id ? 'bg-white sm:bg-transparent text-amber-700 shadow-sm sm:shadow-none' : 'text-slate-500 hover:text-slate-800'
                    }`}
                >
            ...
            {UTILITY_CATEGORIES.find(c => c.id === activeTab)?.subCalculators.map((sub, idx) => {
              ...
              return (
                <div key={idx} className="group relative rounded-3xl glass-card p-6 sm:p-8 border-2 border-slate-200/90 shadow-lg hover:border-amber-400 hover:shadow-xl transition-all duration-300 flex flex-col h-full overflow-hidden bg-white">
                  ...
                  >
                    <span>{sub.id === 'form-121-utility' ? 'Generate Form' : sub.id === 'gstr3b-converter' ? 'Convert Form' : sub.id === 'look-scanned' ? 'Scanned Document' : sub.id === 'tds-challan-converter' ? 'Open Converter' : 'Open Calculator'}</span>
                    <ArrowRight className="w-4 h-4 group-hover/btn:translate-x-1.5 transition-transform" />
                  </Link>
                </div>
              );
