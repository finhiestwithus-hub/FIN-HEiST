...
export default function FloatingWidgets({ isModalOpen, setIsModalOpen, selectedService = '' }: FloatingWidgetsProps) {
  ...
  const handleModalSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setErrorMsg('🔒 Client Account Required: Please Sign In or Create a Free Account to book your consultation.');
      setTimeout(() => {
    ...
    <>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/65 backdrop-blur-md animate-fadeIn">
          <div className="relative w-full max-w-xl bg-gradient-to-br from-white via-slate-50 to-[#FCFBFA] border-2 border-slate-200/90 rounded-3xl shadow-[0_25px_80px_-15px_rgba(245,158,11,0.3)] overflow-hidden animate-card-pulse">
            ...
            <div className="p-7 sm:p-8">
              {isSubmitted ? (
                <div className="py-12 text-center flex flex-col items-center justify-center space-y-4 animate-fadeIn">
                  <div className="w-16 h-16 rounded-full bg-emerald-100 border-2 border-emerald-500 flex items-center justify-center text-emerald-600 animate-bounce shadow-md">
